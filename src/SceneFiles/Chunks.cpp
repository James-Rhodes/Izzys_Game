...
void FlatChunk::GenerateChunk(Vector2 worldCenterPos)
{
    ...
    sceneComponents.push_back(&ecs->CreateEntity<Ground>("Ground" + std::to_string((*terrainCounter)++), Vector2Add(worldCenterPos, {2, -0.5}), 1, 2, terrainBlocks->GetTextureRectangleAtState("Rock")));
};
...
void LadderJumpChunk::GenerateChunk(Vector2 worldCenterPos)
{
    ...
    sceneComponents.push_back(&ecs->CreateEntity<Ground>("Ground" + std::to_string((*terrainCounter)++), Vector2Add(worldCenterPos, {4, -2.8}), 2, 0.5, terrainBlocks->GetTextureRectangleAtState("Rock and Grass")));
